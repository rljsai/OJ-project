...
export const runJavaCode = async (code, input) => {
  const jobId = uuidv4();
  const jobFolder = path.join(baseDir, 'javaJobs', jobId);
  const codePath = path.join(jobFolder, `Main.java`);
  const inputPath = path.join(jobFolder, `${jobId}.in`);
  ...
  const compileResult = await new Promise((resolve) => {
    exec(`javac Main.java`, { cwd: jobFolder, timeout: 10000 }, (err, stdout, stderr) => {
      if (err) {
        resolve({
          success: false,
  ...
  if (!compileResult.success) {
    await cleanUpFiles([codePath, inputPath, path.join(jobFolder, 'Main.class')]);
  ...
  const execResult = await new Promise((resolve) => {
    ...
    child.on('exit', async (code, signal) => {
      await cleanUpFiles([
        codePath,
        inputPath,
        path.join(jobFolder, 'Main.class')
      ]);
