...
const baseDir = path.resolve(__dirname, '../'); // points to /compiler

export const runCppCode = async (code, input) => {
  const jobId = uuidv4();

  const codePath = path.join(baseDir, 'codes', `${jobId}.cpp`);
  ...
  const compileResult = await new Promise((resolve) => {
    exec(`g++ "${codePath}" -o "${executablePath}"`, { timeout: 10000 }, (err, stdout, stderr) => {
      if (err) {
        resolve({
          success: false,
