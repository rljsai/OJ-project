...
export const runGoCode = async (code, input) => {
  const jobId = uuidv4();
  const jobDir = path.join(baseDir, 'goJobs', jobId);
  const codePath = path.join(jobDir, 'main.go');
  const inputPath = path.join(jobDir, `${jobId}.in`);

  // Use .exe extension on Windows
  const isWindows = process.platform === 'win32';
  const binaryName = `${jobId}_exec${isWindows ? '.exe' : ''}`;
  const outputPath = path.join(jobDir, binaryName);
  ...
  const compileResult = await new Promise((resolve) => {
    const buildCommand = `go build -o "${binaryName}" "main.go"`;
    exec(buildCommand, { cwd: jobDir, timeout: 10000 }, (err, stdout, stderr) => {
      if (err) {
        resolve({
          success: false,
